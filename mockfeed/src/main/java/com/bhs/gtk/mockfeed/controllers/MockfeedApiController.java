...
import com.bhs.gtk.mockfeed.api.MockfeedApi;
import com.bhs.gtk.mockfeed.model.TokenRequest;
import com.bhs.gtk.mockfeed.model.TokenResponse;
import com.bhs.gtk.mockfeed.model.TokenResponse.TokenTypeEnum;
...
public class MockfeedApiController implements MockfeedApi {
	...
	public ResponseEntity<byte[]> getAllSymbols(@NotNull @Valid String segment, @NotNull @Valid String name,
		...
		if (user != null) {
			byte[] symbols = stockService.getAllSymbolsAsCSV("nse");
	...
	public ResponseEntity<byte[]> getBars(String bearer, @NotNull @Valid String symbol, @NotNull @Valid String from,
		...
		if (user != null) {
			...
			OffsetDateTime toTime = OffsetDateTime.of(LocalDateTime.parse(to, TIME_FORMATTER),
					ZoneOffset.of("+05:30"));
			byte[] bars = stockService.getBarsAsCSV(symbol, fromTime, toTime);
